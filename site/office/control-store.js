...
function completeness(d){return {accounts:d.accounts.length,emails:d.accounts.filter(x=>x.accountEmail).length,domains:d.domains.length,renewals:d.domains.filter(x=>x.renewalDate).length,tasks:(d.tasks||[]).filter(x=>x.status!=='done').length}}
function actionButtons(collection,id){return `<div class="row-actions"><button class="mini-btn" data-edit="${collection}" data-id="${esc(id)}">editar</button><button class="mini-btn danger" data-delete="${collection}" data-id="${esc(id)}">×</button></div>`}
...
function openRecordForm(collection,id=''){const d=loadControl();const existing=(d[collection]||[]).find(x=>x.id===id);const obj=existing?clone(existing):{id:`${collection}-${Date.now()}`,...clone(TEMPLATES[collection]||{})};const keys=Object.keys(obj).filter(k=>k!=='id');modalRoot.innerHTML=`<div class="modal-backdrop" id="record-bg"><form class="modal record-form" id="record-form"><div class="modal-head"><div><p class="eyebrow">AV OS · INVENTÁRIO</p><h2>${existing?'Editar':'Adicionar'} registro</h2><p class="muted">${esc(collection)}</p></div><button class="close" type="button" id="record-close">×</button></div><div class="form-grid">${keys.map(k=>fieldHtml(k,obj[k])).join('')}</div><div class="form-actions"><button class="btn" type="button" id="record-cancel">Cancelar</button><button class="btn primary" type="submit">Salvar</button></div></form></div>`;const close=()=>modalRoot.innerHTML='';document.getElementById('record-close').onclick=close;document.getElementById('record-cancel').onclick=close;document.getElementById('record-bg').addEventListener('click',e=>{if(e.target.id==='record-bg')close()});document.getElementById('record-form').onsubmit=e=>{e.preventDefault();const f=new FormData(e.currentTarget);for(const k of keys)obj[k]=String(f.get(k)||'').trim();const arr=d[collection]||[];const i=arr.findIndex(x=>x.id===obj.id);if(i>=0)arr[i]=obj;else arr.unshift(obj);d[collection]=arr;saveControl(d);close();views[state.view]?.()}}
