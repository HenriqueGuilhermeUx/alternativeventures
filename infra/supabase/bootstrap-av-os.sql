-- AV OS Data Hub bootstrap
-- Safe to run more than once: all objects use IF NOT EXISTS.
-- Control-plane data only. Do not store product raw sensitive data here.
...
create index if not exists av_deals_updated_idx on av_deals(updated_at desc);
...
-- AV Document Intelligence: operational metadata only.
...
-- Server-side service role is the only access path used by AV OS.
