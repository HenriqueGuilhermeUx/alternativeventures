...
export default async (req)=>{
  ...
  if(req.method==='POST'){
    ...
    const existingR=await sb(`av_deals?venture_slug=eq.${encodeURIComponent(venture)}&company=eq.${encodeURIComponent(company)}&select=id,stage,updated_at&limit=1`);
    if(existingR.ok){const existing=await existingR.json();if(existing.length)return json({created:false,duplicate:true,deal:existing[0]},200)}
    ...
    const r=await sb('av_deals',{method:'POST',body:JSON.stringify(payload)}); if(!r.ok)return json({error:'supabase_error',details:await r.text()},500);
    const rows=await r.json().catch(()=>[]); return json({created:true,deal:rows[0]||null},201);
  }
